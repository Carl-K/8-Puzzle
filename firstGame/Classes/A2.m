...
@implementation A2
@synthesize a2Queue, correctBoard, tableForQueue, tableForPrevious;

-(id) initA2
{
    self.a2Queue = [[queue alloc] initQueue];
...
{
    //NSLog(@"HERE 1");
    [self.a2Queue enqueue:n];
    [self.tableForQueue insertInto:n];
    
    Node *board;
    
    while(![self.a2Queue isEmpty])
    {
        [self.a2Queue sortByFOfNManhattan]; //sort by f(n) manhattan distance <--------------------------------------------------------------------------------------
        
        board = [self.a2Queue dequeue];
    ...
    NSLog(@"LARGEST # NODES HELD: %d", [self.a2Queue getMaxSizeSeen]);
