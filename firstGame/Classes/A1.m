...
@implementation A1
@synthesize a1Queue, correctBoard, tableForQueue, tableForPrevious;

-(id) initA1
{
    self.a1Queue = [[queue alloc] initQueue];
...
{
    //NSLog(@"HERE 1");
    [self.a1Queue enqueue:n];
    [self.tableForQueue insertInto:n];
    
    Node *board;
    
    while(![self.a1Queue isEmpty])
    {
        [self.a1Queue sortByFOfNMisplaced]; //sort by f(n) misplaced, FOUND IN QUEUE CLASS <--------------------------------------------------------------------------------------
        
        board = [self.a1Queue dequeue];
        ...
        if ([board zeroLocation].x > 0)
        {
            ...
            {
                if (![self.tableForQueue checkForDuplicate:temp])
                {
                    [self.tableForQueue insertInto:temp];
                    [self.a1Queue enqueue:temp];
                }
                else
                {
    ...
    NSLog(@"LARGEST # NODES HELD: %d", [self.a1Queue getMaxSizeSeen]);
